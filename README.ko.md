...
**Markdown용 복사**는 저 한 줄을 뷰포트 구간별 변형을 담은 `<picture>`로 감쌉니다.
`<img>` 상자는 남는 높이를 돌려줄 수 없어서 주소 하나로는 항상 가장 좁은 폭의 높이를
...
또는 **SVG 다운로드**로 받아 레포에 커밋하세요. 고정 폭 한 장이라 아래에 빈 공간이
없고, 여느 이미지처럼 어디에나 쓸 수 있으며, 이 서비스와 무관하게 동작합니다.
